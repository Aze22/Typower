...
/// 
/// - OnHover (isOver) is sent when the mouse hovers over a collider2D or moves away.
/// - OnPress (isDown) is sent when a mouse button gets pressed on the collider2D.
...
/// - OnDrag (delta) is sent when a mouse or touch gets pressed on a collider2D and starts dragging it.
/// - OnDrop (gameObject) is sent when the mouse or touch get released on a different collider2D than the one that was being dragged.
/// - OnInput (text) is sent when typing (after selecting a collider2D by clicking on it).
/// - OnTooltip (show) is sent when the mouse hovers over a collider2D for some time without moving.
