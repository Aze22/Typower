...
public class UIScrollBar : UIWidgetContainer
{
			...
			if (mDir != value)
			{
				...
				if (mBG != null)
				{
						...
						(mDir == Direction.Horizontal && width < height))
					{
						...
						if (mBG.collider2D != null) NGUITools.AddWidgetCollider(mBG.gameObject);
						if (mFG.collider2D != null) NGUITools.AddWidgetCollider(mFG.gameObject);
					}
				}
			}
		}
	...
	void Start ()
	{
		if (background != null && background.collider2D != null)
		{
		...
		if (foreground != null && foreground.collider2D != null)
		{
